...
export default function DashboardGuru() {
  ...
  // === AUTH & DATA FETCHING (sama seperti sebelumnya) ===
  useEffect(() => {
  ...
  return (
    <>
      {/* Layout flex supaya footer selalu di bawah */}
      <div className="min-h-screen flex flex-col">

        {/* ===== NAVBAR BARU – KOTAK LURUS (tanpa rounded) ===== */}
      ...
      <span className="absolute -bottom-1 left-0 w-0 h-1 bg-white rounded-full transition-all duration-300 group-hover:w-full"></span>
        ...
        <main className="flex-1 bg-gradient-to-br from-blue-50 to-indigo-100 text-gray-800 p-6 pb-12">
          ...
          <div className="text-center my-12">
            <h2 className="text-3xl font-bold text-[#2C3E50]">Selamat Datang!! ⋆.𐙚,</h2>
                    ...
                    <div className="space-y-5">
                      {pengajuan.filter(p => p.status === "Menunggu").map(p => (
                        <div key={p.id} className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-2xl p-6 flex justify-between items-center">
                          ...
                          <button onClick={() => handleHapus(p.id)} className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-xl font-bold flex items-center gap-3 shadow-lg transition">
